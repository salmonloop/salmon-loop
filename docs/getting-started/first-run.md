...
- Audit logs are written under `<repo>/.salmonloop/runtime/audit/` by default. If `observability.audit.scope` is set to `user`, they are written to `~/.salmonloop/runtime/audit/`.
