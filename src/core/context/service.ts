...
export class ContextService {
  ...
  private async evictLruIfNeeded(): Promise<void> {
    ...
    // Fetch entries once, since this.cacheStore.entries() is expensive (array building)
    ...
    let evictionsNeeded = currentSize - this.cacheMaxEntries;
    let i = 0;
    while (evictionsNeeded > 0 && i < entries.length) {
      const [key] = entries[i];
