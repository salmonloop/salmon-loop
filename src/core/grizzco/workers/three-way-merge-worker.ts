  ...
  private async gitMergeFile(
    ...
    try {
      ...
      await Promise.all([
        fs
          .unlink(tempBase)
          .catch((error) => logIgnoredError(`[ThreeWayMergeWorker] cleanup ${tempBase}`, error)),
        fs
          .unlink(tempTheirs)
          .catch((error) => logIgnoredError(`[ThreeWayMergeWorker] cleanup ${tempTheirs}`, error)),
        fs
          .unlink(tempOurs)
          .catch((error) => logIgnoredError(`[ThreeWayMergeWorker] cleanup ${tempOurs}`, error)),
      ]);
    }
  }
}
