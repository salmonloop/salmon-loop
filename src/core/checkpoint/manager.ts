...
export class CheckpointManager {
  /**
   * Creates a safe snapshot of the current repository state.
  ...
  async createSafeSnapshot(
    ...
    const stagedTree = (await runGit(repoPath, ['write-tree'])).trim();
    ...
    try {
      ...
      if (untracked.length > 0) {
        ...
        await runGit(repoPath, ['add', '--', ...untracked], { env });
      }
      ...
      const workingTree = (await runGit(repoPath, ['write-tree'], { env })).trim();
      ...
      const commitHash = (
        await runGit(repoPath, ['commit-tree', workingTree, '-p', 'HEAD', '-m', metadata], { env })
      ).trim();
  ...
  async restoreToShadow(repoPath: string, shadowPath: string, snapshotHash: string): Promise<void> {
    // 1. Get Metadata
    const msg = await runGit(repoPath, ['log', '-1', '--format=%B', snapshotHash]);
    ...
    await runGit(shadowPath, ['checkout', '-f', snapshotHash]);
    ...
    const parent = await runGit(repoPath, ['rev-parse', `${snapshotHash}^`]);
    const parentTrimmed = parent.trim();
    await runGit(shadowPath, ['reset', '--soft', parentTrimmed]);
    ...
    await runGit(shadowPath, ['read-tree', meta.staged]);
    ...
    try {
      ...
      await runGit(shadowPath, ['update-index', '-q', '--refresh']);
      logger.debug(`[restoreToShadow] Git index refreshed successfully`);

      // Get status for logging (skip untracked files for performance)
      const status = await runGit(shadowPath, ['status', '--short', '-uno']);
      const headRef = await runGit(shadowPath, ['rev-parse', 'HEAD']);
  ...
  async readSnapshotFile(
    ...
    try {
      ...
      const content = await runGit(repoPath, ['show', `${snapshotHash}:${normalized}`]);
      return content;
    } catch (error) {
  ...
  async exportSnapshot(repoPath: string, snapshotHash: string, targetDir: string): Promise<void> {
    ...
    try {
      ...
      await runGit(repoPath, ['read-tree', snapshotHash], { env });
      ...
      await runGit(repoPath, ['checkout-index', '-a', '--prefix', prefix], { env });
    } finally {
      // Cleanup temporary index
      try {
        await rm(tempIndexFile, { force: true });
  ...
  async restoreToMain(
    ...
    await runGit(repoPath, ['reset', '--soft', parent.trim()]);

    // 5. Restore Working Tree
    // Checkout files from snapshot commit into working directory
    await runGit(repoPath, ['checkout', snapshotHash, '--', '.']);

    // 6. Restore Index
    // Reset index to match the staged tree from metadata
    await runGit(repoPath, ['read-tree', meta.staged]);
  }
  ...
  async deleteSnapshot(repoPath: string, snapshotHash: string): Promise<void> {
    ...
    try {
      await runGit(repoPath, ['update-ref', '-d', `refs/s8p/snapshots/${snapshotHash}`]);
      return;
    } catch {
    ...
    if (target) {
      await runGit(repoPath, ['update-ref', '-d', target.ref]);
    } else {
  ...
  async clearSnapshots(repoPath: string): Promise<void> {
    ...
    for (const snapshot of snapshots) {
      ...
      await runGit(repoPath, ['update-ref', '-d', snapshot.ref]);
    }
  }
  ...
  async getSnapshotDetails(
    ...
    const stagedOutput = await runGit(repoPath, [
      'diff',
      '--name-only',
      `${snapshotHash}^`,
      meta.staged,
    ...
    const unstagedOutput = await runGit(repoPath, [
      'diff',
      '--name-only',
      meta.staged,
      snapshotHash,
  ...
  async getSnapshotFileContent(
    ...
    return await runGit(repoPath, ['show', `${snapshotHash}:${filePath}`]);
  }
  ...
  private async getSafeUntrackedFiles(repoPath: string): Promise<string[]> {
    ...
    const output = await runGit(repoPath, ['ls-files', '-o', '--exclude-standard']);
  ...
  async getSnapshotFiles(repoPath: string, snapshotHash: string): Promise<string[]> {
    const output = await runGit(repoPath, ['ls-tree', '-r', '--name-only', snapshotHash]);
    return output.split('\n').filter((f) => f.trim());
  }
