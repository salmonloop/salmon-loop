export const en = {
  ...
  configLogUsage: 'Usage: /config log <full|standard|compact>',
  configLogSuggestion: (view: string) => `Set log view to ${view}`,
  configLogCurrent: (view: string) => `Current UI log view: ${view}`,
  configLogInvalid: (view: string) =>
    `Invalid log view: ${view}. Expected one of: full, standard, compact.`,
  configLogUpdated: (view: string) => `UI log view updated: ${view}`,
  configLogPersisted: (path: string) => `UI log view saved to ${path}`,
  configLogPersistFailed: (error: string) => `Failed to save UI log view: ${error}`,
  authUsage:
