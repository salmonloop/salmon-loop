...
export const CommandSuggestionList: React.FC<CommandSuggestionListProps> = React.memo(
  ({ suggestions, selectedIndex, parentCommand }) => {
    ...
    const maxNameLength = React.useMemo(
      () => suggestions.reduce((max, s) => Math.max(max, s.name.length), 0),
      [suggestions],
    );
