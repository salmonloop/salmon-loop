    ...
    authTokens.length > 0
      ? (req: any, res: any, next: any) => {
          ...
          if (scheme?.toLowerCase() === 'bearer' && token) {
            const hashedInput = crypto.createHash('sha256').update(token).digest();
            for (const authToken of authTokens) {
              const hashedAuthToken = crypto.createHash('sha256').update(authToken).digest();
              if (crypto.timingSafeEqual(hashedInput, hashedAuthToken)) {
