      ...
      ? (req: any, res: any, next: any) => {
          ...
          if (scheme?.toLowerCase() === 'bearer' && token) {
            ...
            for (const authToken of authTokens) {
              ...
              const hashedToken = crypto.createHash('sha256').update(tokenBuffer).digest();
              const hashedAuthToken = crypto.createHash('sha256').update(authTokenBuffer).digest();
              if (crypto.timingSafeEqual(hashedToken, hashedAuthToken)) {
