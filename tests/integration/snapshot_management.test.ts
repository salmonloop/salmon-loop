...
describe('Snapshot Management Integration (CLI V2 Support)', () => {
  ...
  it('should create a snapshot with custom message and metadata', async () => {
    ...
    const refExists = await runGit(repoPath, [
      'rev-parse',
      '--verify',
    ...
    const commitMsg = await runGit(repoPath, ['log', '-1', '--format=%B', result.commitHash]);
  ...
  it('should export snapshot content to target directory', async () => {
    ...
    await runGit(repoPath, ['add', '.']); // Stage it to ensure tree structure
  ...
  it('should delete a specific snapshot', async () => {
    ...
    await expect(
      runGit(repoPath, ['rev-parse', '--verify', `refs/s8p/snapshots/${snap.commitHash}`]),
    ).rejects.toThrow();
  });
