...
function createAgentDispatchToolstack(
  routerCall: (envelope: any) => Promise<any>,
  allowedPhases: string[] = ['PLAN'],
): any {
...
describe('Grizzco steps: PLAN/PATCH tool calling path', () => {
  ...
  it('PLAN exposes same-attempt recent read artifacts from toolCallingAudit', async () => {
    const captured: any[][] = [];
    const llm: LLM = {
      getCapabilities: () => ({ toolCalling: true }),
      createPlan: mock(async () => {
      ...
      chat: mock(async (messages: any) => {
        ...
        return {
          ...
          content: JSON.stringify({
            goal: 'retry-goal',
            files: ['src/index.js'],
            changes: ['Inspect recent read artifacts'],
            verify: 'bun -e "process.exit(0)"',
          }),
        };
      }),
    ...
    const ctx: any = {
      ...
      options: {
        llm,
        instruction: 'continue after fs.read in explore',
        dryRun: true,
      },
      toolCallingAudit: [
        {
          ...
          toolIntent: 'READ',
          toolResultStatus: 'ok',
          toolResultReadArtifactPath: 'src/readme.ts',
    ...
    const lastUserMessage = captured[0][captured[0].length - 1];
    expect(lastUserMessage.role).toBe('user');
    expect(lastUserMessage.content).toContain('s8p://artifact/recent-read-123');
    expect(lastUserMessage.content).toContain('src/readme.ts');
    expect(lastUserMessage.content).toContain('artifact.read');
  });

  it('PLAN exposes same-attempt tool result preview artifacts from toolCallingAudit', async () => {
    const captured: any[][] = [];
    const llm: LLM = {
      getCapabilities: () => ({ toolCalling: true }),
      createPlan: mock(async () => {
      ...
      chat: mock(async (messages: any) => {
        ...
        return {
          ...
          content: JSON.stringify({
            goal: 'retry-goal',
            files: ['src/index.js'],
            changes: ['Inspect tool result previews'],
            verify: 'bun -e "process.exit(0)"',
          }),
        };
      }),
    ...
    const ctx: any = {
      ...
      options: {
        llm,
        instruction: 'continue after large tool result',
        dryRun: true,
      },
      toolCallingAudit: [
        {
    ...
    const lastUserMessage = captured[0][captured[0].length - 1];
    expect(lastUserMessage.role).toBe('user');
    expect(lastUserMessage.content).toContain('s8p://artifact/tool-preview-123');
    expect(lastUserMessage.content).toContain('Tool result preview: web.search output');
    expect(lastUserMessage.content).toContain('artifact.read');
  });
  ...
  it('PATCH forwards runtime contextSnapshot into agent_dispatch execution context', async () => {
    let receivedRuntimeCtx: any;
    const routerCall = mock(async (envelope: any) => {
      receivedRuntimeCtx = envelope.ctx;
      return {
    ...
    const llm: LLM = {
      ...
      createPlan: mock(async () => ({
        goal: 'test-goal',
        files: ['src/index.js'],
        changes: ['Use sub-agent context in patch'],
        verify: 'bun -e "process.exit(0)"',
      })),
      createPatch: mock(async () => {
        throw new Error('createPatch should not be called when tool calling is enabled');
    ...
    const conversationContext = [{ role: 'assistant' as const, content: 'prior answer' }];
    const artifactHints = {
      verifyArtifact: {
        handle: 's8p://artifact/verify-patch-ctx',
        mimeType: 'text/plain',
        sha256: 'verify',
        size: 12,
      },
    ...
    const planRuntime = { sessionId: 'plan-patch-1', planPathHint: '.salmonloop/plan.md' };

    const ctx: any = {
      workspace: { workPath: repo.path, baseRepoPath: 'C:\\repo-base', strategy: 'worktree' },
      options: {
        llm,
        instruction: 'test',
        dryRun: true,
      ...
      planRuntime,
      context: { primaryFile: 'src/index.js', primaryText: 'const x = 1;' },
      contextResult: { prompt: 'ASSEMBLED_CONTEXT_PATCH', meta: { contextHash: 'ctx-patch' } },
      plan: {
        goal: 'test-goal',
        files: ['src/index.js'],
        changes: ['Use sub-agent context in patch'],
      ...
      emit: () => {},
      toolstack: createAgentDispatchToolstack(routerCall, ['PATCH']),
    };
