...
describe('ACP formal protocol (SDK)', () => {
  ...
  it('paginates session/list with opaque cursors', async () => {
    const { clientConn } = createConnectedPair({
      toAgent: (conn) =>
        createAcpFormalAgent({
          conn,
          agentInfo: { name: 'salmon-loop', version: '0.2.0' },
          sessionStorePolicy: { maxEntries: 80 },
          facade: {
            createTask: async () => {
              throw new Error('not used');
            },
    ...
    const firstPage = await clientConn.listSessions({ cwd: '/repo' });
    expect(firstPage.sessions).toHaveLength(50);
    expect(typeof firstPage.nextCursor).toBe('string');

    const secondPage = await clientConn.listSessions({
      cwd: '/repo',
  ...
  it('rejects invalid or mismatched session/list cursors', async () => {
    const { clientConn } = createConnectedPair({
      toAgent: (conn) =>
        createAcpFormalAgent({
          conn,
    ...
    await expect(clientConn.listSessions({ cursor: 'not-a-valid-cursor' })).rejects.toMatchObject({
      code: -32602,
    });

    const page = await clientConn.listSessions({ cwd: '/repo' });
    await expect(
      clientConn.listSessions({ cwd: '/other-repo', cursor: page.nextCursor }),
    ).rejects.toMatchObject({ code: -32602 });
  });

  it('deletes a session from future list, load, and resume operations', async () => {
    ...
    await clientConn.unstable_deleteSession({ sessionId });
    await clientConn.unstable_deleteSession({ sessionId });

    expect(cancelledTasks).toEqual(['task_delete_1']);
  ...
  describe('invalid config value strings', () => {
    it('rejects unsupported permission policy string value', async () => {
      const { clientConn } = createConnectedPair({
        toAgent: (conn) =>
          createAcpFormalAgent({
            conn,
  ...
  describe('setSessionMode validation', () => {
    it('rejects unsupported modeId', async () => {
      const { clientConn } = createConnectedPair({
        toAgent: (conn) =>
          createAcpFormalAgent({
            conn,
      ...
      const { sessionId } = await clientConn.newSession({ cwd: '/repo', mcpServers: [] });
      await expect(
        clientConn.setSessionMode({ sessionId, modeId: 'invalid_mode' }),
      ).rejects.toMatchObject({ code: -32602 });
    });
  });

  describe('prompt on non-existent session', () => {
    it('returns -32004 when prompting a non-existent session', async () => {
      const { clientConn } = createConnectedPair({
        toAgent: (conn) =>
          createAcpFormalAgent({
            conn,
  ...
  describe('userMessageId on slash command', () => {
    it('echoes userMessageId when handling a slash command', async () => {
      const { clientConn } = createConnectedPair({
        toAgent: (conn) =>
          createAcpFormalAgent({
            conn,
      ...
      const response = await clientConn.prompt({
        sessionId,
        prompt: [{ type: 'text', text: '/help' }],
        messageId: 'msg_slash_789',
      });

      expect(response.stopReason).toBe('end_turn');
      expect(response.userMessageId).toBe('msg_slash_789');
    });
  });

  describe('unknown content block type', () => {
    it('rejects unsupported content block type', async () => {
      const { clientConn } = createConnectedPair({
        toAgent: (conn) =>
          createAcpFormalAgent({
            conn,
      ...
      await expect(
        clientConn.prompt({
          sessionId,
          prompt: [{ type: 'video', src: 'http://example.com/video.mp4' } as any],
        }),
      ).rejects.toMatchObject({ code: -32602 });
    });
  });
