...
describe('chatWithToolsStreaming', () => {
  ...
  it('forwards runtime contextSnapshot to agent_dispatch in streaming tool execution', async () => {
    ...
    const dispatchSpec: ToolSpec<any, any> = {
      name: 'agent_dispatch',
      source: 'builtin',
      intent: 'AGENT',
    ...
    const envelope = routerSpy.mock.calls[0]?.[0];
    expect(envelope?.ctx?.phase).toBe(Phase.PLAN);
    expect(envelope?.ctx?.contextSnapshot).toEqual({
      conversationContext,
      artifactHints,
      toolCallingAudit,
      planRuntime,
